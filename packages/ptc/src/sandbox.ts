import vm from "vm";
import { isValidToolName, sanitizeToolName } from "./signature-generator";
import type {
	ConsoleCapture,
	SandboxContext,
	SandboxOptions,
...
export class NodeSandbox {
	...
	async execute(
		...
		try {
			...
			const console: ConsoleCapture = {
				log: (...args: any[]) => {
					const message = args
						.map((arg) => this.formatLogArgument(arg))
						.join(" ");
					logs.push(message);
				},
				error: (...args: any[]) => {
			...
			const sandboxContext: SandboxContext = {
				...
				checkCancellation: () => {
					if (
						typeof (sandboxContext as any).__cancelled !== "undefined" &&
						(sandboxContext as any).__cancelled
					) {
						throw new Error("Execution was cancelled");
					}
				},
			...
			for (const [originalName, tool] of Object.entries(tools)) {
				...
				toolProxies[safeName] = async (args: any) => {
					...
					try {
						...
						if (!tool.execute) {
							throw new Error(
								`Tool ${originalName} does not have an execute function`,
							);
						}
						...
						const abortPromise = new Promise<never>((_, reject) => {
							...
							this.activeAbortController?.signal.addEventListener(
								"abort",
								onAbort,
							);
							...
							toolPromise.finally(() => {
								this.activeAbortController?.signal.removeEventListener(
									"abort",
									onAbort,
								);
							});
						...
						if (
							...
							console.error(
								`Tool execution failed for ${originalName}: ${errorMessage}`,
							);
						}
			...
			if (
				executionError.message.includes("cancelled") ||
				executionError.message.includes("abort")
			) {
		...
		return new Promise((resolve, reject) => {
			...
			timeoutId = setTimeout(() => {
				if (!isCompleted) {
					cleanup();
					reject(
						new Error(`Execution timeout: exceeded ${this.timeout}ms limit`),
					);
				}
			}, this.timeout);
        ...
        `;
				...
				const timeoutPromise = new Promise((_, timeoutReject) => {
					setTimeout(() => {
						timeoutReject(
							new Error(
								`Async execution timeout: exceeded ${this.timeout}ms limit`,
							),
						);
					}, this.timeout);
				});
	...
	private validateEnvironment(): void {
		...
		if (this.timeout > 30000) {
			console.warn(
				"Warning: Timeout exceeds 30 seconds, which may impact performance",
			);
		}
